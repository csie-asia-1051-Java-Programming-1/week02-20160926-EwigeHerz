package hw;
import java.util.*;
/*
* Topic: 假設銅板有1元、5元、50元共三種，媽媽請小明去菜市場買水果，給了小明N元，且媽媽交待，
* 要老闆找小明的零錢的數目要最少，而小明到了水果攤買了a1顆蘋果，a2顆柳丁，及a3顆桃子，
* 1顆蘋果15元，1顆柳丁20元，1顆桃子30元，請問老問需找多少個1元、5元、50元，其銅板數目最少。
* (先輸入媽媽給小明多少錢，N，接著輸入a1, a2, a3 ， 在此n, a1, a2, a3為整數，且a1 *15+ a2*20+ a3*30小於或等於N。
* 列出共找小明多少個1元，5元及50元，若帶的錢不夠買水果，則顯示”0”)
* Date: 2016/09/26
* Author: 105021027 陳宴湘
*/

public class hw01_105021027 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
    Scanner sc = new Scanner(System.in);
    System.out.print("媽媽給小明多少錢：");
    int n = sc.nextInt();
    System.out.print("蘋果:");
    int a = sc.nextInt();
    System.out.print("柳丁:");
    int o = sc.nextInt();
    System.out.print("桃子:");
    int p = sc.nextInt();
    int m = 0;
    m = n - (a*15+o*20+p*30);
    System.out.println(m);
    if(m<=0){
    	System.out.print(0);
    }
    if(m>=0 && m/50>0 ){
    	System.out.println("50:"+m/50);
    	
        }
        	if(m>=0 && m%50/5>0){
        System.out.println("5:"+m%50/5);
        
        }
        	if(m>=0 && m%50%5>0){
        System.out.println("1:"+m%50%5);
        	}
    
    
	}
}

